...
public class FinanceMbtiController {

    private final FinanceMbtiService financeMbtiService;

    // 1) 문항 조회
    @GetMapping("/questions")
    public List<MbtiQuestionResponseDto> getQuestions() {
        return financeMbtiService.getActiveQuestions().stream()
    ...
    public FinanceMbtiTestResultResponseDto submitTest(@Valid @RequestBody FinanceMbtiTestRequestDto req) {
        MemberMbtiTest saved = financeMbtiService.submitTest(req);
    ...
    public FinanceMbtiTestResultResponseDto getResult(@RequestParam Long memberId) {
        return FinanceMbtiTestResultResponseDto.from(financeMbtiService.getCurrentResult(memberId));
    }

    // 4) 결과 유형 조회 (16유형 목록)
    @GetMapping("/result/type")
    public List<FinanceMbtiTypeInfoDto> getResultTypes() {
        return financeMbtiService.getTypeInfos();
    }
}
